...
import AddEditManageCharges from './AddEditFuelCategories'; // Import the image
...
const ManageCharges: React.FC<ManageUserProps> = ({ postData, getData, isLoading }) => {
    ...
    const fetchData = async () => {
        try {
            ...
            let apiUrl = `/fuel/category?page=${currentPage}`;
            if (searchTerm) {
    ...
    const isAddPermissionAvailable = UserPermissions?.includes("fuel-category-create");
    const isListPermissionAvailable = UserPermissions?.includes("fuel-category-list");
    const isEditPermissionAvailable = UserPermissions?.includes("fuel-category-edit");
    const isEditSettingPermissionAvailable = UserPermissions?.includes("fuel-category-setting");
    const isDeletePermissionAvailable = UserPermissions?.includes("fuel-category-delete");
    const isAssignAddPermissionAvailable = UserPermissions?.includes("fuel-category-assign-permission");
        ...
        <>
            ...
            <div className="panel mt-6">
                ...
                <div className="flex md:items-center md:flex-row flex-col mb-5 spacebetween">
                    <h5 className="font-bold text-lg dark:text-white-light">Fuel Categories</h5>
                    {showFilterOptions && (
                        <SearchBar
